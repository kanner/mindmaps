...
\newpage
\section{Data structures and algorithm design}
...
\subsubsection{Lazy Unions}
\underline{Raison d`entre}: maintain a partition of a set $X$

\begin{description}
	\item{(FIND)} given $x \in X$, return name of $x$`s group
	\item{(UNION)} given $x | y$, merge groups containing them\\
\end{description}

\textbf{Previous solution} (for Kruskal`s MST algorithm):
\begin{itemize}
	\item each $x \in X$ points directly to the "leader" of its group
	\item $O(1)$ FIND [just return $x$`s leader]
	\item $O(n \log n)$ total work for $n$ UNIONs [when 2 groups merge, smaller group inherits leader of larger one]
\end{itemize}

\textbf{Lazy union} idea: update only \underline{one} pointer each merge!
...
\begin{tikzpicture}[scale=0.9]
  ...
  % vertices
  \node[vertex] (a) at  (1.5,2) {1};
  \node[vertex] (b) at  (0,0) {2};
  \node[vertex] (c) at  (3,0) {3};
  \node[vertex] (d) at  (4,1) {+};
  \node[vertex] (e) at  (6.5,2) {4};
  \node[vertex] (f) at  (8,0) {6};
  \node[vertex] (g) at  (5,0) {5};
  \node[vertex] (h) at  (9,1) {=};

  \node[vertex] (i) at  (13,2) {4};
  \node[vertex] (j) at  (14.5,0) {5};
  \node[vertex] (k) at  (16,0) {6};
  \node[vertex] (l) at  (10,0) {1};
  \node[vertex] (m) at  (11.5,0) {2};
  \node[vertex] (n) at  (13,0) {3};
  % edges
  \draw[edge] (b) to (a);
  \draw[edge] (c) to (a);
  \draw[edge] (f) to (e);
  ...
  \draw[edge] (n) to (i);
  % cycle edge
  \path (a) edge[my loop] node[above] {} (a);
  \path (e) edge[my loop] node[above] {} (e);
  \path (i) edge[my loop] node[above] {} (i);
\end{tikzpicture}
...
\begin{tikzpicture}[scale=0.9]
  ...
  \node[vertex] (j) at  (14.5,0) {5};
  \node[vertex] (k) at  (16,0) {6};
  \node[vertex] (l) at  (11.5,0) {\color{red} 1};
  \node[vertex] (m) at  (10,-2) {\color{red} 2};
  \node[vertex] (n) at  (13,-2) {\color{red} 3};
  % edges
  \draw[edge] (b) to (a);
  \draw[edge] (c) to (a);
  \draw[edge] (f) to (e);
...
\begin{tikzpicture}
    ...
    \node at (4.5,2.5) {\small 4};
	\node at (4.5,1.5) {\small 5};
    \node at (5.5,2.5) {\small 4};
	\node at (5.5,1.5) {\small 6};
    ...
    \draw (13,4) -- (13,5);
    \draw (14,4) -- (14,5);
	\node at (8.5,4.5) {\small \color{red} 4};
	\node at (8.5,3.5) {\small 1};
    \node at (9.5,4.5) {\small \color{red} 4};
	\node at (9.5,3.5) {\small 2};
    \node at (10.5,4.5) {\small \color{red} 4};
	\node at (10.5,3.5) {\small 3};
    \node at (11.5,4.5) {\small 4};
	\node at (11.5,3.5) {\small 4};
    \node at (12.5,4.5) {\small 4};
	\node at (12.5,3.5) {\small 5};
    \node at (13.5,4.5) {\small 4};
	\node at (13.5,3.5) {\small 6};
    ...
    \draw (13,0) -- (13,1);
    \draw (14,0) -- (14,1);
	\node at (8.5,0.5) {\small \color{red} 4};
	\node at (8.5,-0.5) {\small 1};
    \node at (9.5,0.5) {\small 1};
	\node at (9.5,-0.5) {\small 2};
    \node at (10.5,0.5) {\small 1};
	\node at (10.5,-0.5) {\small 3};
    \node at (11.5,0.5) {\small 4};
	\node at (11.5,-0.5) {\small 4};
    \node at (12.5,0.5) {\small 4};
	\node at (12.5,-0.5) {\small 5};
    \node at (13.5,0.5) {\small 4};
	\node at (13.5,-0.5) {\small 6};
\end{tikzpicture}

\underline{Merge in general}: when two groups merge in a UNION, make one group`s leader (i.e., root of the tree) a child of the other one.
\\

\textbf{Pro`s}: UNION reduces to $2$ FINDS [$r_1 = FIND(x), r_2 = FIND(y)$] and $O(1)$ extra work [link $r_1$ and $r_2$ together]
\\
...
\begin{tikzpicture}[scale=0.9]
  ...
  % vertices
  \node[vertex] (a) at  (1.5,2) {1};
  \node[vertex] (b) at  (0,0) {0};
  \node[vertex] (c) at  (1.5,0) {0};
  \node[vertex] (d) at  (3,0) {0};

  \node[vertex] (e) at  (8,2) {2};
  \node[vertex] (f) at  (6.5,0) {1};
  \node[vertex] (g) at  (9.5,0) {0};
  \node[vertex] (h) at  (5,-2) {0};
  \node[vertex] (i) at  (8,-2) {0};
  % edges
  \draw[edge] (b) to (a);
  \draw[edge] (c) to (a);
  \draw[edge] (d) to (a);
...
\begin{tikzpicture}[scale=0.9]
  ...
  \node[vertex] (a) at  (1.5,2) {1};
  \node[vertex] (b) at  (0,0) {0};
  \node[vertex] (c) at  (3,0) {0};
  \node[vertex] (d) at  (4,1) {+};
  \node[vertex] (e) at  (6.5,2) {1};
  \node[vertex] (f) at  (8,0) {0};
  \node[vertex] (g) at  (5,0) {0};
  \node[vertex] (h) at  (9,1) {=};
  
  \node[vertex] (i) at  (13,2) {2};
  \node[vertex] (j) at  (14.5,0) {0};
  \node[vertex] (k) at  (16,0) {0};
  \node[vertex] (l) at  (11.5,0) {1};
  \node[vertex] (m) at  (10,-2) {0};
  \node[vertex] (n) at  (13,-2) {0};
  % edges
  \draw[edge] (b) to (a);
  \draw[edge] (c) to (a);
  \draw[edge] (f) to (e);
...
\begin{corollary}worst-case running time of FIND, UNION with Union by rank is $O(\log n)$\end{corollary}


\subsubsection{Path compression}
\textbf{Idea}: why bother traversing a leaf-root path multiple times?

\underline{Path compression}: after $FIND(x)$, install shortcuts (i.e., rewire parent pointers) to $x$`s root all along the $x \rightarrow root$ path

\begin{tikzpicture}[scale=0.9]
  \tikzset{vertex/.style = {shape=circle,draw,minimum size=1.5em}}
  \tikzset{edge/.style = {->,> = latex'}}
  % vertices
  \node[vertex] (a) at  (3,2) {7};
  \node[vertex] (b) at  (4.5,0) {5};
  \node[vertex] (c) at  (3,-2) {2};
  \node[vertex] (d) at  (6,-2) {3};
  \node[vertex] (e) at  (1.5,0) {6};
  \node[vertex] (f) at  (0,-2) {4};
  \node[vertex] (g) at  (-1.5,-4) {1};
  \node[vertex] (h) at (7, 0) {$\rightarrow$};
  \node[vertex] (i) at  (11,2) {7};
  \node[vertex] (j) at  (12.5,0) {5};
  \node[vertex] (k) at  (11,-2) {2};
  \node[vertex] (l) at  (14,-2) {3};
  \node[vertex] (m) at  (11.5,0) {6};
  \node[vertex] (n) at  (9.5,0) {1};
  \node[vertex] (o) at  (10.5,0) {4};
  % edges
  \draw[edge] (b) to (a);
  \draw[edge] (e) to (a);
  \draw[edge] (c) to (b);
...
\begin{tikzpicture}
	...
	\node at (1.5,1.5) {\small 2};
    \node at (2.5,2.5) {\small 5};
	\node at (2.5,1.5) {\small 3};
    \node at (3.5,2.5) {\small 6};
	\node at (3.5,1.5) {\small 4};
    \node at (4.5,2.5) {\small 7};
	\node at (4.5,1.5) {\small 5};
    \node at (5.5,2.5) {\small 7};
	\node at (5.5,1.5) {\small 6};
	\node at (6.5,2.5) {\small 7};
	\node at (6.5,1.5) {\small 7};

    \node at (8,2) {\small $\rightarrow$};
    ...
    \draw (15,2) -- (15,3);
	\draw (16,2) -- (16,3);
	\node at (9.5,2.5) {\small \color{red} 7};
	\node at (9.5,1.5) {\small 1};
    \node at (10.5,2.5) {\small 5};
	\node at (10.5,1.5) {\small 2};
    \node at (11.5,2.5) {\small 5};
	\node at (11.5,1.5) {\small 3};
    \node at (12.5,2.5) {\small \color{red} 7};
	\node at (12.5,1.5) {\small 4};
    \node at (13.5,2.5) {\small 7};
	\node at (13.5,1.5) {\small 5};
    \node at (14.5,2.5) {\small 7};
	\node at (14.5,1.5) {\small 6};
	\node at (15.5,2.5) {\small 7};
	\node at (15.5,1.5) {\small 7};
\end{tikzpicture}

\textbf{Pro`s}: constant factor overhead to FIND (from "multitasking")
\\

\textbf{Con`s}: speeds up subsequent FINDs (but how much?)
\\

\underline{Important}: maintain all rank fields EXACTLY as without path compression
\begin{itemize}
	\item ranks initially all $0$
	\item in UNION, new root $=$ old root with bigger rank
	\item when merging two nodes of common rank $r$, reset new root`s rank to $(r+1)$
\end{itemize}
...
\begin{tikzpicture}[scale=0.9]
  ...
  \tikzset{edge/.style = {->,> = latex'}}
  % vertices
  \node[vertex] (a) at  (3,2) {3};
  \node[vertex] (b) at  (4.5,0) {1};
  \node[vertex] (c) at  (3,-2) {0};
  \node[vertex] (d) at  (6,-2) {0};
  \node[vertex] (e) at  (1.5,0) {2};
  \node[vertex] (f) at  (0,-2) {1};
  \node[vertex] (g) at  (-1.5,-4) {0};
  \node[vertex] (h) at (7, 0) {$\rightarrow$};
  \node[vertex] (i) at  (11,2) {3};
  \node[vertex] (j) at  (12.5,0) {1};
  \node[vertex] (k) at  (11,-2) {0};
  \node[vertex] (l) at  (14,-2) {0};
  \node[vertex] (m) at  (11.5,0) {2};
  \node[vertex] (n) at  (9.5,0) {0};
  \node[vertex] (o) at  (10.5,0) {1};
  % edges
  \draw[edge] (b) to (a);
  \draw[edge] (e) to (a);
  \draw[edge] (c) to (b);
...
\textbf{Bad news}: now $rank[x]$ is only an upper bound on the maximum number of hops on a path from a leaf to $x$ (which could be much less)
\\

\textbf{Good news}: Rank lemma still holds ($\leq \frac{n}{2^r}$ objects with rank $r$) and still always have $rank[parent(x)] > rank[x]$ for all non-roots $x$

\begin{theorem}[Hopcroft-Ullman, 73] with Union by Rank and path compression, $m$ Union+Find operations take $O(m \log^* n)$ time, where $\log^* n =$ number of times you need to apply $\log$ to $n$ before the result is $\leq 1$ [will focus on interesting case where $m = \Omega(n)$]\end{theorem}

\textbf{Note}: $\log^* (2^{65536}) = 5$. In general $\log^* (2^{2^{2^{\dots^{2}}}})$ = t (i.e., $\log^*$ of $t$ times tower function)
\\

\textbf{Intuition}: installing shortcuts should significantly speed up subsequent FINDs + UNIONs

\underline{Question}: How to track this progress and quantify the benefit?
\\
